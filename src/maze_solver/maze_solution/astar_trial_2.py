...
# Load the maze image
image_path = '/home/kris/maze_opencv/datasets/8.png'  # Replace with your image path
...
# Define the output grid size (11x11)
output_grid_size = 11
...
print("Modified 11x11 Grid World:")
print(grid_world)
...
# Function to get start and goal positions based on the border openings
def get_start_goal(grid_world):
    # Search the top row for the goal position (first '1' in the row)
    for j in range(output_grid_size):
        if grid_world[0, j] == 1:  # Goal position (top row)
            goal = (0, j)
            break

    # Search the bottom row for the start position (first '1' in the row)
    for j in range(output_grid_size):
        if grid_world[output_grid_size-1, j] == 1:  # Start position (bottom row)
            start = (output_grid_size-1, j)
            break

    return start, goal

# Define the start and goal positions dynamically
